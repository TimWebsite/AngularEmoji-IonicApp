...
function checkDragEvent(a) {
    if (!a || a.target && ("IMG" == a.target.tagName || "A" == a.target.tagName)) return !1;
    if (!a.dataTransfer || !a.dataTransfer.types) return !0;
...
emojiApp.config(["$sceProvider", function (a) {
...
}]), emojiApp.directive("contenteditable", ["$sce", function () {
    return {
        restrict: "A",
        require: "?ngModel",
    ...
    function e(b, e) {
        ...
        function g() {
            p && ($(p).trigger("change"), h())
        }

        function h() {
            var a = p.offsetHeight;
            t != a && (t = a, b.$emit("ui_editor_resize"))
        }
        ...
        function k(a) {
            return 9 != a.keyCode || a.shiftKey || a.ctrlKey || a.metaKey || $modalStack.getTop() ? void 0 : (o.focus(), cancelEvent(a))
        }
        var l = $("textarea", e)[0],
            m = $("input", e),
            n = $("#emojibtn", e)[0],
            ...
            q = d($("#messageDiv"));
        if ($("#messageDiv").replaceWith(q(b)), p) {
            o = p, $(p).addClass("form-control"), $(p).attr("placeholder", c($(l).attr("placeholder"))(b));
            var r;
            $(p).on("DOMNodeInserted", i).on("keyup", function () {
                h(), u || b.$apply(function () {
                    b.emojiMessage.messagetext = p.textContent
                }), a.cancel(r), r = a(g, 1e3)
            })
        }
        var s = !0;
        $(o).on("keydown", function (c) {
            if (p && h(), 13 == c.keyCode) {
                var d = !1;
                if (s && !c.shiftKey ? d = !0 : s || !c.ctrlKey && !c.metaKey || (d = !0), d) return a.cancel(r), g(), b.emojiMessage.replyToUser(), f(), cancelEvent(c)
            }
        });
        var t = p.offsetHeight;
        $(document).on("keydown", k), $(document).on("paste", j);
        var u = !1;
        b.$on("$destroy", function () {
            $(document).off("paste", j), $(document).off("keydown", k), $(submitBtn).off("mousedown"), m.off("change"), p && $(p).off("DOMNodeInserted keyup", i), $(o).off("keydown")
        })
    }
    return {
        scope: {
